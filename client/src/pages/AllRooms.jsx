...
return (
    <label className='flex gap-3 items-center cursor-pointer mt-2 text-sm'>
        <input 
        type="checkbox" checked={selected} onChange={(e)=>onChange(e.target.checked, label)}/>
...
return (
    <label className='flex gap-3 items-center cursor-pointer mt-2 text-sm'>
        <input 
        type="radio" name='sortOption' checked={selected} onChange={()=>onChange(label)}/>
        <span className='font-light select-none'>{label}</span>
    </label>
)}
const AllRooms = () => {
...
<div className='flex flex-col items-start text-left'>
    ...
    <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'> Take advantage of our limited-time offers and special packages to enhance your stay and 
        create unforgatable memories.
    </p>
</div>
{
       ...
       border-b border-gray-300 last:pb-30 last:border-0'> 
           ...
           <div className='md:w-1/2 flex flex-col gap-2'>
            ...
            <div className='flex items-center'>
             ...
             <img key={index} 
            //  average rating
              src={room.rating > index? assets.starIconFilled:assets.starIconOutlined}/>
          ))}
            </div>
            <div className='flex items-center text-gray-500 gap-1 mt-2 text-sm'>
